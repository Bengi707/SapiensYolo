...
def get_cpu_info():
    ...
    from ultralytics.ultralytics.utils import PERSISTENT_CACHE  # avoid circular import error
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from ultralytics.ultralytics.utils.torch_utils import strip_optimizer
...
def profile(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Example:
        ```python
        from ultralytics.ultralytics.utils.torch_utils import profile
