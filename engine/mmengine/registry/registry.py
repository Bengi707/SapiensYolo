...
from engine.mmengine.utils import get_object_from_string, is_seq_of
...
class Registry:
    ...
    def switch_scope_and_registry(self, scope: Optional[str]) -> Generator:
        ...
        Examples:
            >>> from engine.mmengine.registry import Registry, DefaultScope, MODELS
            >>> import time
            >>> # External Registry
    ...
    def build(self, cfg: dict, *args, **kwargs) -> Any:
        ...
        Examples:
            >>> from engine.mmengine import Registry
            >>> MODELS = Registry('models')
            >>> @MODELS.register_module()
            >>> class ResNet:
